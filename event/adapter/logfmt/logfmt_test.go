...
func TestPrint(t *testing.T) {
	...
	for _, test := range []struct {
		...
		name:   "namespace",
		event:  event.Event{Source: event.Source{Space: "github.com/fromelicks/exp/event"}},
		expect: `in=github.com/fromelicks/exp/event`,
	}, {
		name:   "at",
...
func TestPrinterFlags(t *testing.T) {
	...
	for _, test := range []struct {
		...
		event: event.Event{
			Source: event.Source{Space: "github.com/fromelicks/exp/event"},
			Labels: []event.Label{event.String("msg", "some text")},
		},
		before: `in=github.com/fromelicks/exp/event msg="some text"`,
		after:  `msg="some text"`,
	}} {
		t.Run(test.name, func(t *testing.T) {
			buf.Reset()
